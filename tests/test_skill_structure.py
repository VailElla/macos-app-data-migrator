...
class SkillStructureTests(unittest.TestCase):
    ...
    def test_low_space_and_finder_safety_contract_is_present(self) -> None:
        ...
        self.assertIn("如果用户明确要求 Codex 代办", skill)
        self.assertIn("不能把改名再次交还给用户", skill)
        self.assertIn("If the user explicitly asks Codex to do it", skill)
        self.assertIn("must not hand the rename back to the user", skill)
